...
export const getAllLoanAuctionMeta = async () => {
	...
	try {
		...
		const emptyRemoved = data.filter((loan: any) => loan.nftType !== null) as unknown as LoanAuction[]
		loanAuctions.set(emptyRemoved)
		console.log({ result })
	} catch (e) {
		console.log(e);
	}
...
export const createLoanAuction = async (
	nftID: number,
	duration: number,
	yield_: number,
...
export const basketTxs = {
	depositFT: async (baskedId: string, storagePath: string, amount: string) => {
		...
		console.log({ cadence });
		transactionStatus.set(
			`depositing ${amount} of tokens from ${storagePath} to basket #${baskedId}`
		);
	...
	depositFTs: async (baskedId: string, storagePaths: string[], amounts: string[]) => {
		...
		console.log({ cadence });
		transactionStatus.set(
			`depositing ${JSON.stringify(amounts)} of tokens from ${JSON.stringify(
				storagePaths
			)} to basket #${baskedId}`
		);
	...
	depositNFTs: async (
		...
		console.log({ cadence });
		transactionStatus.set(
			`depositing ${JSON.stringify(ids)} tokens from ${storagePath} to basket #${baskedId}`
		);
	...
	bulkDepositNFTs: async (
		...
		transactionStatus.set(
			`depositing ${JSON.stringify(ids)} tokens from ${JSON.stringify(
				storagePaths
			)} to basket #${baskedId}`
		);
...
export const sendTokens = async (recipient: string, amount: string) => {
  ...
  `;
	const args = (arg: (arg0: string, arg1: string) => any, t: { Address: any; UFix64: any }) => [
